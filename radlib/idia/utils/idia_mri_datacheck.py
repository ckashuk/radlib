...
class MRI_DataCheck:
    ...
    def browse_nifti_modalities(self, time_format='dates', summary=False, modalities_list=False, write_csv=True):
        ...
        for patient_id in os.listdir(root_dir):
            ...
            if os.path.isdir(patient_dir):
                ...
                for timepoint, interval, interval_tag, date in zip(mri_studies, intervals, intervals_name,
                                                                   cumulative_dates):
    ...
    def to_construct_queryFile(self):
        """
...
def getting_images_list_from_csv(root_path, mri_data, subdir, mri_mod, file_xlsx_list, df_key, mask_str,
                                 ID_filtering=False, time_filtering=False, time_point=None, pipeline=None,
                                 print_search=False):
    ...
    imgs = sorted(imgs)
    segs = sorted(segs)
    print('\n number of images files: {}, segmentation files: {} for MRI modality: {}, from : {} '.format(len(imgs),
                                                                                                          len(segs),
                                                                                                          mri_mod,
                                                                                                          xlsx_path))
