...
def evaluate_at_continuous_index_wrapper(img:sitk.Image, new_point: np.ndarray, default_value:float=0, mode=sitk.sitkLinear)-> Union[int, float]:
    """
    ...
    try:
        new_value = img.EvaluateAtContinuousIndex(new_point, interp=mode)
...
def generate_volume_projection(img:sitk.Image, grid:np.ndarray=None, default_value:float=0, mode=sitk.sitkLinear) -> np.ndarray:
    """
...
def generate_image_projection(img:sitk.Image, grid:np.ndarray=None, mode=sitk.sitkLinear, copy_tags=True) -> sitk.Image:
    """
   ...
   img: sitk.Image
    ...
    new_volume = generate_volume_projection(img, grid, mode)
